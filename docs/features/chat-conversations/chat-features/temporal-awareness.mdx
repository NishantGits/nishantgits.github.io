...
For an AI to be truly helpful, it needs to understand the concept of time. FlowChat ensures that models are aware of the current date, time, and timezone so they can provide contextually relevant answers (e.g., "What's on my schedule today?" or "Summarize my meetings from yesterday").

## System-Level Awareness

By default, FlowChat injects temporal variables into the model's environment via the system prompt. Even without specialized tools, most models are aware of:
