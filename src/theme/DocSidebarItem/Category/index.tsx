...
function CategoryLinkLabel({label, logo}: {label: string, logo?: string}) {
  return (
    <>
      {logo && (
        <img
...
function DocSidebarItemCategoryCollapsible({
  ...
  const logo = (item.customProps as { logo?: string } | undefined)?.logo;
  const {
    docs: {
    ...
    <li
      ...
      <div
        ...
        <Link
          ...
          {...props}>
          <CategoryLinkLabel label={label} logo={logo} />
        </Link>
        {href && collapsible && (
          <CollapseButton
            collapsed={collapsed}
